package org.yazgel.jpql.xtext.serializer;
...
public class JPQLSemanticSequencer extends AbstractDelegatingSemanticSequencer {
		...
		if(semanticObject.eClass().getEPackage() == JPQLPackage.eINSTANCE) switch(semanticObject.eClass().getClassifierID()) {
			...
			case JPQLPackage.FROM_CLASS:
				if(context == grammarAccess.getFromClassIdRule()) {
				   ...
				   context == grammarAccess.getFromEntryRule()) {
					sequence_FromClass_FromClassId_FromClassVar(context, (FromClass) semanticObject); 
					return; 
				}
				else break;
			...
			case JPQLPackage.FUNCTION:
				if(context == grammarAccess.getFunctionRule()) {
					sequence_Function_NumericFunction_StringFunction(context, (Function) semanticObject); 
					return; 
				}
	...
	protected void sequence_DateTimeExpression(EObject context, DateTimeExpression semanticObject) {
		...
		INodesForEObjectProvider nodes = createNodeProvider(semanticObject);
		SequenceFeeder feeder = createSequencerFeeder(semanticObject, nodes);
		feeder.accept(grammarAccess.getDateTimeExpressionAccess().getValueLITERAL_TEMPORALParserRuleCall_0(), semanticObject.getValue());
		feeder.finish();
	}
	 ...
	 */
	protected void sequence_Function_NumericFunction_StringFunction(EObject context, Function semanticObject) {
		genericSequencer.createSequence(context, semanticObject);
	}
